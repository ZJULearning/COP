...
class PruneMobileNetForImagenet(PruneBase):
    def __init__(self, weights_dict, **prune_args):
        super(PruneMobileNetForImageNet, self).__init__(weights_dict, **prune_args)
    ...
    def _get_output_size_from_layer_name(self, name):
        ...
        for i in range(2, 4): # block2~3
            if ("block%d" % i) in name:
                return [self.image_size[0] // 4, self.image_size[1] // 4]
        if "block1" in name or "conv0" in name:
